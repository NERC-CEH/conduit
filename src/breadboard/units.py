"""Runtime unit validation helpers for the breadboard DAG.

Each model node owns its own unit declarations via the
`breadboard.dag._utils.declare_units` decorator (``input_units`` / ``output_units``);
...
MODE_ENV_VAR = "BREADBOARD_UNITS_MODE"
...
EXACT_ENV_VAR = "BREADBOARD_UNITS_EXACT"
...
def set_mode(mode: str | None) -> None:
    ...
    ``BREADBOARD_UNITS_MODE`` environment variable) applies.
    """
...
def get_mode() -> Mode:
    ...
    Resolution order: ``BREADBOARD_UNITS_MODE`` environment variable, then the
...
def set_exact_match(exact: bool | None) -> None:
    ...
    Passing ``None`` clears the process override so the default (or the
    ``BREADBOARD_UNITS_EXACT`` environment variable) applies.
    """
...
def get_exact_match() -> bool:
    ...
    Resolution order: ``BREADBOARD_UNITS_EXACT`` environment variable, then the
...
def assert_valid_unit(unit: str, context: str) -> None:
    ...
    offending site (e.g. ``"mymodel input 'vpd_weekly'"``) for the message.
...
def units_compatible(a: str, b: str) -> bool:
    ...
    (`breadboard.dag.unit_check.check_dag_units`). Both strings are assumed
...
def units_from_signature(
    ...
    This is the single source the runtime `breadboard.dag._utils.declare_units`
