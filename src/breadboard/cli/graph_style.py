"""Styling configuration for the ``breadboard graph`` DAG visualisation.
...
``breadboard graph --style <file>`` whose keys are deep-merged over these defaults,
...
class GraphvizSpec:
    """Resolved styling for a single ``breadboard graph`` invocation.
