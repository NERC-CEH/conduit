...
def make_style_function(spec: GraphvizSpec, output_vars: set[str]) -> StyleFunction:
    ...
    def custom_style(
        *, node: graph_types.HamiltonNode, node_class: str
        ...
        if node.tags.get("module") == "breadboard.inputs.static":
...
def _node_maps(dr: "Driver") -> tuple[dict[str, str], dict[str, str]]:
    ...
    :func:`breadboard.units.units_from_signature` — the same single source the unit
...
def graph(
    ...
    default styling (see ``breadboard.cli.graph_style``).
