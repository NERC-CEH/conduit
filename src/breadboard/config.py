"""Configuration management for breadboard."""
...
class CacheSpec:
    ...
    def from_config(cls, entry: dict) -> "CacheSpec":
        ...
        return cls(
            path=entry.get("path", ".breadboard_cache"),
...
class SubsetSpec:
    ...
    independent ``breadboard run`` processes can each handle a different spatial
...
class Config:
    ...
    def _parse_graphviz(self, data: dict, driver_config: dict) -> list[str]:
        ...
        DAG-visualisation styling lives in its own file passed to ``breadboard
        graph --style`` (see ``breadboard.cli.graph_style``), not in the science
