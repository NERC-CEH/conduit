...
`breadboard.units.units_from_signature` — the same single source the runtime
...
executing any node — the basis of ``breadboard run --dry-run``. Inputs routed through
...
def _collect_unit_maps(
    ...
    resample_edges = {
        ...
        for node in hg.nodes
        if node.tags.get("module") == "breadboard.dag.resample"
...
def check_input_units(
    ...
    *,
    ...
    `breadboard.units.check_units` the executing DAG uses — so it raises/warns
