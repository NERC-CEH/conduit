...
def declare_units(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    `breadboard.units.units_from_signature`: parameters annotated
    ...
    1. validates/converts each declared ``DataArray`` input to its unit via
       `breadboard.units.check_units`, honouring the active mode
       (`breadboard.units.get_mode`); validation is skipped entirely in ``off`` mode;
