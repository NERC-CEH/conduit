"""Runtime unit validation helpers for the conduit DAG.

Each model node owns its own unit declarations via the
`conduit.dag._utils.declare_units` decorator (``input_units`` / ``output_units``);
...
MODE_ENV_VAR = "CONDUIT_UNITS_MODE"
...
EXACT_ENV_VAR = "CONDUIT_UNITS_EXACT"
...
def set_mode(mode: str | None) -> None:
    ...
    ``CONDUIT_UNITS_MODE`` environment variable) applies.
    """
...
def get_mode() -> Mode:
    ...
    Resolution order: ``CONDUIT_UNITS_MODE`` environment variable, then the
...
def set_exact_match(exact: bool | None) -> None:
    ...
    Passing ``None`` clears the process override so the default (or the
    ``CONDUIT_UNITS_EXACT`` environment variable) applies.
    """
...
def get_exact_match() -> bool:
    ...
    Resolution order: ``CONDUIT_UNITS_EXACT`` environment variable, then the
...
def units_compatible(a: str, b: str) -> bool:
    ...
    (`conduit.dag.unit_check.check_dag_units`). Both strings are assumed
...
def units_from_signature(
    ...
    This is the single source the runtime `conduit.dag._utils.declare_units`
